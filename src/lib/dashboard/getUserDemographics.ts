...
begin
  ...
  -- AGE GROUPS
    ...
    if user_set = 'TOTAL' then
      ...
      from public.active_session s
      join public."user" u on s.user_id = u.user_id
      where u.birth_date is not null and age(u.birth_date) < interval '18 years'
      union all
      select '18-25'::text, count(*)::int, '#DD6B20'::text
      from public.active_session s
      join public."user" u on s.user_id = u.user_id
      where u.birth_date is not null and age(u.birth_date) between interval '18 years' and interval '25 years'
      union all
      select '26-40'::text, count(*)::int, '#2B6CB0'::text
      from public.active_session s
      join public."user" u on s.user_id = u.user_id
      where u.birth_date is not null and age(u.birth_date) between interval '26 years' and interval '40 years'
      union all
      select '41-60'::text, count(*)::int, '#38B2AC'::text
      from public.active_session s
      join public."user" u on s.user_id = u.user_id
      where u.birth_date is not null and age(u.birth_date) between interval '41 years' and interval '60 years'
      union all
      select '60+'::text, count(*)::int, '#E53E3E'::text
      from public.active_session s
      join public."user" u on s.user_id = u.user_id
      where u.birth_date is not null and age(u.birth_date) > interval '60 years';
    end if;

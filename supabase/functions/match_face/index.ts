// Face Match Edge Function for EduScan
// Extracted from eduscan-kiosk/src/lib/faceid/getFaceMatch.ts
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({
        error: "Failed to connect to Face ID Service. Please coordinate to system administrator."
      }), {
        status: 502,
        headers: corsHeaders
      })
    ...
    try {
      ...
      return new Response(JSON.stringify({
        error: "There was an error connecting to FaceID service. Please try again."
      }), {
        status: 502,
        headers: corsHeaders
      })
    ...
    if (sessionError) {
      ...
      return new Response(JSON.stringify({
        error: "Failed to retrieve session. Please coordinate to system administrator."
      }), {
        status: 500,
        headers: corsHeaders
      })

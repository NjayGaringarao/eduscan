// Logging Edge Function for EduScan
// Extracted from eduscan-kiosk/src/lib/faceid/setUserLog.ts
...
const sendMessage = async (user: any, action: string) => {
  const now = new Date()
...
serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("user")
      .select("user_id, first_name, middle_name, last_name, sex, employee(type, division, title), student(department, program), guardian(first_name, middle_name, last_name, sex, contact_number)")
      .eq("user_id", user_id)
      .single()
    ...
    // Step 7: Return appropriate values
    if (data?.employee) {
      return new Response(JSON.stringify({

  ...
  return (
    <div className={cn("relative", containerClassName)}>
      {title && (
        ...
        >
          <p>{title} </p>
          {isRequired === true && <p className="text-uRed"> *</p>}
        </div>
      )}
      ...
      <div className="relative flex-1">
        <input
          ...
          className={cn(
            "border border-textbody w-full rounded-lg p-2 focus:border-2 hover:border-2 text-primary",
            inputClassName,
            isValueInvalid &&
              "border-uRed/50 focus:border-uRed hover:border-uRed/90",
            isPassword && "pr-12"
          )}
          value={value ?? ""}
